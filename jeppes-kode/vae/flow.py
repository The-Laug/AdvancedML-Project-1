...
class MaskedCouplingLayer(nn.Module):
    ...
    def __init__(self, scale_net, translation_net, mask):
        ...
        self.inverse_mask = nn.Parameter(torch.ones_like(self.mask) - self.mask, requires_grad=False)
    ...
    def forward(self, z):
        ...
        sum_log_det_J: [torch.Tensor]
            The sum of the log determinants of the Jacobian matrices of the forward transformations of dimension `(batch_size,)`.
        """

        untouched = z * self.mask
        s = self.scale_net(untouched)
        t = self.translation_net(untouched)
        z_prime = untouched + self.inverse_mask * (z * torch.exp(s) + t)

        log_det_J = torch.sum(s * self.inverse_mask, -1)
    ...
    def inverse(self, z_prime):
        ...
        untouched = z_prime * self.mask
        s = self.scale_net(untouched)
        t = self.translation_net(untouched)
        z = untouched + self.inverse_mask * ( (z_prime - t) * torch.exp(-s))

        log_det_invJ = torch.sum(self.inverse_mask * (-s), -1)
...
if __name__ == "__main__":
    ...
    if args.data == "mnist":
        ...
        train_loader = torch.utils.data.DataLoader(data, batch_size=args.batch_size, shuffle=True, collate_fn=collate_images)
        test_loader = torch.utils.data.DataLoader(test_data, batch_size=args.batch_size, shuffle=False, collate_fn=collate_images)
    ...
    if args.mode == 'train':
        ...
        with torch.no_grad():
            if args.data == 'mnist':
                # Generate a single MNIST sample and save as image
                sample = model.sample((1,)).cpu().view(28, 28)
                save_image(sample, 'mnist.png')
            else:
