...
def train(model, optimizer, data_loader, epochs, device):
    ...
    for epoch in range(epochs):
        ...
        for x in data_iter:
            # handle datasets that return (inputs, labels)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mask-type', type=str, default='checkerboard', choices=['checkerboard', '5050', 'checker_random'], help='type of mask to use for coupling layers (default: %(default)s)')
    ...
    if not args.data == 'mnist':
        ...
        train_loader = torch.utils.data.DataLoader(datasets.MNIST(root='data', train=True, download=True, transform=transforms.Compose([
                            transforms.ToTensor(),
    ...
    # Handle datasets that return (images, labels) tuples (e.g., MNIST)
    first_batch = next(iter(train_loader))
    if isinstance(first_batch, (list, tuple)):
        first_batch = first_batch[0]
    # ensure shape is (batch, D) before reading feature dim
    D = first_batch.reshape(first_batch.shape[0], -1).shape[1]
    base = GaussianBase(D)
    ...
    #Every other elemet should be masked 
    mask = torch.zeros((D,))
    if args.mask_type == 'checkerboard':
        mask[::2] = 1
    elif args.mask_type == '5050':
        mask = (torch.rand_like(mask) > 0.5).float()
    elif args.mask_type == 'checker_random':
        mask[::2] = 1
        mask = (mask.bool() ^ (torch.rand_like(mask) > 0.5)).float()


    for i in range(num_transformations):
        mask = (1-mask) # Flip the mask
        scale_net = nn.Sequential(nn.Linear(D, num_hidden), nn.ReLU(), nn.Linear(num_hidden, num_hidden), nn.ReLU(), nn.Linear(num_hidden, D))
    ...
    if args.mode == 'train':
        ...
        if not args.data == 'mnist':
            ...
            # samples shape is (n_samples, D) because MNIST was flattened
            D = samples.shape[1]
            side = int(math.sqrt(n_samples))
            samples = samples.reshape(n_samples, 28, 28)

            fig, axes = plt.subplots(side, side, figsize=(side, side), dpi=100)
